...
where "COACH_LINK_SKILL".skill_id = (select id from "SKILL" s where name = '重訓')
...
SELECT 
  ...
  COUNT(*) AS "銷售數量"
FROM 
  "CREDIT_PURCHASE" AS cp
INNER JOIN 
  "CREDIT_PACKAGE" AS cp2 ON cp2.id = cp.credit_package_id
WHERE 
  cp.created_at >= '2024-11-01 00:00:00' 
  AND cp.created_at <= '2024-11-30 23:59:59'
GROUP BY 
  cp2.name;
...
WHERE 
  cb.created_at >= '2024-11-01 00:00:00' 
  AND cb.created_at <= '2024-11-30 23:59:59'
  AND cb.status != '課程已取消';